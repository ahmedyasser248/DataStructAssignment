...
public class PlayersFinder implements IPlayersFinder{
    ...
    public int thresholdCheck(char[][] arrCharT,int x , int y , int team){
        int area=4;
        arrCharT[y][x]=' ';
        if(x>maxX)maxX=x;if(y>maxY)maxY=y;if(x<minX)minX=x;if(y<minY)minY=y;
        if(x+1 < arrCharT[0].length && arrCharT[y][x + 1] == team + 48)area = thresholdCheck(arrCharT, x+1, y, team)+area;
        if(y+1 < arrCharT.length && arrCharT[y+1][x] == team + 48)area =  thresholdCheck(arrCharT, x , y+1, team)+area;
        if(x-1 >= 0 && arrCharT[y][x - 1] == team + 48)area = thresholdCheck(arrCharT, x-1, y, team)+area;
        if(y-1>= 0 && arrCharT[y-1][x] == team + 48)area =  thresholdCheck(arrCharT, x , y-1, team)+area;
        return area;
    }
