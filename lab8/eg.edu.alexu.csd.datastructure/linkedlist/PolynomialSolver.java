...
public class PolynomialSolver implements IPolynomialSolver {
  ...
  public int[][] change2(DLinkedList Result) {
  ...
  boolean validString(String input) {
    input = input + ",";
    if (Pattern.matches("(\\s*\\(\\s*-?[0-9]+\\s*\\,\\s*-?[0-9]+\\s*\\)\\s*\\,\\s*)+", input)) {
      return true;
    } else {
      return false;
    }
  ...
  public int[][] change(String input) {
    ...
    if (count1 % 2 != 0) {
      System.out.println("There is an incomplete term");
      return null;
  ...
  public int[][] add(char poly1, char poly2) {
    Node temp1 = polynomials
        .get(poly1);//head of first linked list,will be used to traverse the linkedlist.
  ...
  public int[][] multiply(char poly1, char poly2) {
    ...
    int[][] productArray = change2(Result);
